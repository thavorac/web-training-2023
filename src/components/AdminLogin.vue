          ...
          <div class="col-6">
            <form @submit.prevent="loginAdmin" class="shadow-lg p-3 bg-white rounded-4 float-end">
              ...
              <div class="container">
                ...
                <input v-model="admin.email" type="email" placeholder="Enter Email" name="email" required>
  
                <label for="password"><b>Password</b></label>
                <input v-model="admin.password" type="password" placeholder="Enter Password" name="password" required>
...
const loginAdmin = async () => {
  try {
    const { data } = await axios.post('http://localhost:80/api/admin/login', admin.value);
