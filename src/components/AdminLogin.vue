<template>
      ...
      <div class="bg-img">
        <div class="container position-absolute">
          <div class="row justify-content-end">
            <div class="col-6">
              <form @submit.prevent="loginData" class="shadow-lg p-3 bg-white rounded-4 float-end">
  ...
  const loginData = () => {
    axios.post('/admin/login', user.value)
      .then(({ data }) => {
        if (data.message === 'Login successful') {
          store.dispatch('login', { user: data.admin, token: data.token });
