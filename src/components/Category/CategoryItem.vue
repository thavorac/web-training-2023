...
import { ref, toValue, toRefs, computed } from 'vue';
import IconEdit from '../icons/IconEdit.vue';
import IconDelete from '../icons/IconDelete.vue';
import IconDetail from '../icons/IconDetail.vue';
import CreateFormCategory from '../../components/Category/CreateFormCategory.vue'; // Import the CreateFormCategory component
...
const editCategory = (categoryId: number) => {
    router.push({ path: `/categories/${categoryId}/edit` });
};
...
//     if (startDate.value) {
//         filtered = filtered.filter(category => new Date(category.created_at) >= startDate.value);
//     }

//     if (endDate.value) {
//         filtered = filtered.filter(category => new Date(category.updated_at) <= endDate.value);
//     }
...
<template>
    <CategoryView :subtitle="'Category'">
        <div class="w-full selection:bg-gray-100 py-10 px-10 mt-10  rounded-lg bg-gray-200  shadow-md">
                        ...
                        <div class="bg-gray-100 flex items-center py-3 px-3 space-x-4 rounded-md">
                            ...
                            <template v-else>
                                <div class="flex flex-col ">
                                    ...
                                    <span cla.ss=" font-sans font-semibold text-2xl">{{ filteredData.length }}</span>
                                </div>
                            </template>
                            ...
                            <tbody>
                                ...
                                <template v-else>
                                    <tr v-for="(category, index) in filteredData"
                                        ...
                                        :key="index">
                                        <td scope="row" class="px-6 py-6 text-gray-800">
                                            {{ index + 1 }}
                                        </td>
                                        <td scope="row" class="px-6 py-6  text-gray-800">
                                            {{ category?.name }}
                                        ...
                                        <td class="px-6 py-6 flex space-x-2">
                                            <!-- <RouterLink :to="{ path: `/categories/${category.id}/edit` }"> -->
                                            <IconEdit @click="handleEdit"
            ...
            <div class="text-xl text-right font-semibold text-black">
