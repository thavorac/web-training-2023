...
const viewRecipe = async (orderId) => {
  try {
    const response = await axios.get(`http://localhost/api/orders/${orderId}`);
    const recipeId = response.data.recipe.id; // Adjust based on your API response
      ...
      <table class="w-full">
        ...
        <tbody>
          ...
          >
            ...
            <td class="py-3 px-2">
              <span
                class="bg-[rgb(50,212,61)] text-white text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-blue-200 dark:text-blue-800"
                >{{ order.status }}</span
              >
            </td>
            <td class="py-3 px-2">
