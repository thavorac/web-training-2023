        ...
        <table class="w-full overflow-auto text-sm text-left rtl:text-right text-gray-500">
          ...
          <thead class="text-xs text-gray-700 bg-gray-50">
            <tr>
              ...
              <th scope="col" class="px-6 py-3 text-lg font-sans">Discount Type</th>
          ...
          <tbody>
            ...
            <!-- Data Rows -->
            <template v-else>
              <tr v-for="(history, index) in filteredData" :key="index"
                ...
                <td class="px-6 py-6">{{ history.product_name }}</td>
                <td class="px-6 py-6">{{ history.discount_type }}</td>
                <td class="px-6 py-6">{{ history.date_range }}</td>
              </tr>
            </template>
          </tbody>
        </table>
...
import BackIcon from '../icons/BackIcon.vue';
  ...
  const fetchDiscountHistory = async () => {
    try {
      const response = await axios.get('http://localhost:80/api/discount-history');
      discountHistory.value = response.data.discount_history;
    } catch (error) {
  ...
  const filteredData = computed(() => {
    ...
    return discountHistory.value.filter((history) => {
      const historyStartDate = new Date(history.start_date).getTime();
      const historyEndDate = new Date(history.end_date).getTime();
