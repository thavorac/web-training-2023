...
const cartStore = useCartStore(); // Rename this to avoid confusion
...
// Reactive data from the cart store
const countCartItems = ref(0);

const fetchCartItemCount = async () => {
  try {
    const response = await axios.get('/api/cart-item-count'); // Ensure this matches the route
    countCartItems.value = response.data.count;
  } catch (error) {
  ...
  <div class="container-fluid bg-gray-900 box-border">
    <div class="row">
      ...
      <div class="col-md-4 d-flex justify-evenly align-items-center">
        ...
        <router-link class="no-underline" to="/cart">
          <SignIn text="Cart">
            <IconCard />
            ({{ countCartItems }})
          </SignIn>
